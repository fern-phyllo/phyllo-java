...
@JsonDeserialize(
    using = CreateSdkTokenException.Deserializer.class
)
public final class CreateSdkTokenException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateSdkTokenException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateSdkTokenException other(Object unknownValue, int statusCode) {
    return new CreateSdkTokenException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateSdkTokenException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateSdkTokenException> {
    @Override
    public CreateSdkTokenException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateSdkTokenException(value, statusCode);
    }
  }
}
