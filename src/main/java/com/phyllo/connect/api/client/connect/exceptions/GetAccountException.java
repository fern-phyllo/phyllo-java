...
@JsonDeserialize(
    using = GetAccountException.Deserializer.class
)
public final class GetAccountException extends Exception {
  private final Value value;

  private int statusCode;

  private GetAccountException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetAccountException other(Object unknownValue, int statusCode) {
    return new GetAccountException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetAccountException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetAccountException> {
    @Override
    public GetAccountException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetAccountException(value, statusCode);
    }
  }
}
