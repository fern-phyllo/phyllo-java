...
@JsonDeserialize(
    using = GetUserException.Deserializer.class
)
public final class GetUserException extends Exception {
  private final Value value;

  private int statusCode;

  private GetUserException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetUserException other(Object unknownValue, int statusCode) {
    return new GetUserException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetUserException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetUserException> {
    @Override
    public GetUserException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetUserException(value, statusCode);
    }
  }
}
