...
@JsonDeserialize(
    using = DisconnectAccountException.Deserializer.class
)
public final class DisconnectAccountException extends Exception {
  private final Value value;

  private int statusCode;

  private DisconnectAccountException(Value value, int statusCode) {
    this.value = value;
  ...
  public static DisconnectAccountException other(Object unknownValue, int statusCode) {
    return new DisconnectAccountException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "DisconnectAccountException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<DisconnectAccountException> {
    @Override
    public DisconnectAccountException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new DisconnectAccountException(value, statusCode);
    }
  }
}
